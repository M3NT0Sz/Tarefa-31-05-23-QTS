...
{
    public partial class AniversariantesUsuarios : Form
    {
        string cnsql = "";
        public AniversariantesUsuarios(string cn)
        {
            cnsql = cn;
            InitializeComponent();
        }
        ...
        private void consultar()
        {
            ...
            string sql = "select * from USUARIOS where dn between'" + dateTimePicker1.Value.ToString("yyyy-MM-dd") + "'and'" + dateTimePicker2.Value.ToString("yyyy-MM-dd") + "' order by login";//envniando uma instrução em sql para o banco
